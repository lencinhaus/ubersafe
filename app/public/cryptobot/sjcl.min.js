...
c;d++){for(e=2;e*e<=d;e++)if(0===d%e)continue a;8>c&&(this.m[2*c]=a(Math.pow(d,0.5)),this.m[2*c+1]=b(Math.pow(d,0.5))<<24|this.ra[c]);this.d[2*c]=a(Math.pow(d,1/3));this.d[2*c+1]=b(Math.pow(d,1/3))<<24|this.ta[c];c++}},k:function(a){var b,c,d=a.slice(0),e=this.h,f=this.d,g=e[0],h=e[1],k=e[2],l=e[3],n=e[4],m=e[5],q=e[6],t=e[7],s=e[8],E=e[9],G=e[10],F=e[11],da=e[12],Q=e[13],ea=e[14],R=e[15],y=g,w=h,J=k,H=l,K=n,I=m,X=q,L=t,z=s,x=E,S=G,M=F,T=da,N=Q,Y=ea,O=R;for(a=0;80>a;a++){if(16>a)b=d[2*a],c=d[2*a+
1];else{c=d[2*(a-15)];var u=d[2*(a-15)+1];b=(u<<31|c>>>1)^(u<<24|c>>>8)^c>>>7;var A=(c<<31|u>>>1)^(c<<24|u>>>8)^(c<<25|u>>>7);c=d[2*(a-2)];var B=d[2*(a-2)+1],u=(B<<13|c>>>19)^(c<<3|B>>>29)^c>>>6,B=(c<<13|B>>>19)^(B<<3|c>>>29)^(c<<26|B>>>6),Z=d[2*(a-7)],$=d[2*(a-16)],P=d[2*(a-16)+1];c=A+d[2*(a-7)+1];b=b+Z+(c>>>0<A>>>0?1:0);c+=B;b+=u+(c>>>0<B>>>0?1:0);c+=P;b+=$+(c>>>0<P>>>0?1:0)}d[2*a]=b|=0;d[2*a+1]=c|=0;var Z=z&S^~z&T,fa=x&M^~x&N,B=y&J^y&K^J&K,ka=w&H^w&I^H&I,$=(w<<4|y>>>28)^(y<<30|w>>>2)^(y<<25|w>>>
7),P=(y<<4|w>>>28)^(w<<30|y>>>2)^(w<<25|y>>>7),la=f[2*a],ga=f[2*a+1],u=O+((z<<18|x>>>14)^(z<<14|x>>>18)^(x<<23|z>>>9)),A=Y+((x<<18|z>>>14)^(x<<14|z>>>18)^(z<<23|x>>>9))+(u>>>0<O>>>0?1:0),u=u+fa,A=A+(Z+(u>>>0<fa>>>0?1:0)),u=u+ga,A=A+(la+(u>>>0<ga>>>0?1:0)),u=u+c,A=A+(b+(u>>>0<c>>>0?1:0));c=P+ka;b=$+B+(c>>>0<P>>>0?1:0);Y=T;O=N;T=S;N=M;S=z;M=x;x=L+u|0;z=X+A+(x>>>0<L>>>0?1:0)|0;X=K;L=I;K=J;I=H;J=y;H=w;w=u+c|0;y=A+b+(w>>>0<u>>>0?1:0)|0}h=e[1]=h+w|0;e[0]=g+y+(h>>>0<w>>>0?1:0)|0;l=e[3]=l+H|0;e[2]=k+J+(l>>>
0<H>>>0?1:0)|0;m=e[5]=m+I|0;e[4]=n+K+(m>>>0<I>>>0?1:0)|0;t=e[7]=t+L|0;e[6]=q+X+(t>>>0<L>>>0?1:0)|0;E=e[9]=E+x|0;e[8]=s+z+(E>>>0<x>>>0?1:0)|0;F=e[11]=F+M|0;e[10]=G+S+(F>>>0<M>>>0?1:0)|0;Q=e[13]=Q+N|0;e[12]=da+T+(Q>>>0<N>>>0?1:0)|0;R=e[15]=R+O|0;e[14]=ea+Y+(R>>>0<O>>>0?1:0)|0}};sjcl.hash.sha1=function(a){a?(this.h=a.h.slice(0),this.e=a.e.slice(0),this.c=a.c):this.reset()};sjcl.hash.sha1.hash=function(a){return(new sjcl.hash.sha1).update(a).finalize()};
...
this.S++;this.d=sjcl.hash.sha256.hash(this.d.concat(e));this.P=new sjcl.cipher.aes(this.d);for(d=0;4>d&&!(this.q[d]=this.q[d]+1|0,this.q[d]);d++);}for(d=0;d<a;d+=4)0===(d+1)%this.ia&&ba(this),e=ca(this),c.push(e[0],e[1],e[2],e[3]);ba(this);return c.slice(0,a)},setDefaultParanoia:function(a,b){0===a&&"Setting paranoia=0 will ruin your security; use it only for testing"!==b&&p("Setting paranoia=0 will ruin your security; use it only for testing");this.Q=a},addEntropy:function(a,b,c){c=c||"user";var d,
...
sjcl.random.addEntropy(a,2,"accelerometer");U(0)}};function ha(a,b){var c,d=sjcl.random.O[a],e=[];for(c in d)d.hasOwnProperty(c)&&e.push(d[c]);for(c=0;c<e.length;c++)e[c](b)}function U(a){window&&window.performance&&"function"===typeof window.performance.now?sjcl.random.addEntropy(window.performance.now(),a,"loadtime"):sjcl.random.addEntropy((new Date).valueOf(),a,"loadtime")}function ba(a){a.d=ca(a).concat(ca(a));a.P=new sjcl.cipher.aes(a.d)}
function ca(a){for(var b=0;4>b&&!(a.q[b]=a.q[b]+1|0,a.q[b]);b++);return a.P.encrypt(a.q)}function D(a,b){return function(){b.apply(a,arguments)}}sjcl.random=new sjcl.prng(6);
a:try{var ia,ja,V;if("undefined"!==typeof module&&module.exports&&(ja=require("crypto"))&&ja.randomBytes)ia=ja.randomBytes(128),sjcl.random.addEntropy(ia,1024,"crypto['randomBytes']");else if(window&&Uint32Array){V=new Uint32Array(32);if(window.crypto&&window.crypto.getRandomValues)window.crypto.getRandomValues(V);else if(window.msCrypto&&window.msCrypto.getRandomValues)window.msCrypto.getRandomValues(V);else break a;sjcl.random.addEntropy(V,1024,"crypto['getRandomValues']")}}catch(ma){console.log("There was an error collecting entropy from the browser:"),
console.log(ma)}
...
return this};d.inverse=function(){return this.power(this.modulus.sub(2))};c.fromBits=sjcl.bn.fromBits;return c};var W=sjcl.bn.pseudoMersennePrime;
